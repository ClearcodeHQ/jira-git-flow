    ...
    def _get_formatted_choices(self):
        ...
        for i, choice in enumerate(self.choices):
            ...
            if choice[2]:  # disabled
                choices.append(('class:default', '- %s' % (choice[0],)))
            else:
                if selected:
                    choices.append(('class:sel_issue', '\u25cf '))
                else:
                    choices.append(('class:default', '\u25cb '))
                ...
                choices.append(('class:badge', '***'))
                choices.append(('class:end', 'end'))
            choices.append(('class:default', '\n'))

        return choices


def question(choices, pointer_index):
    ...
    style = Style.from_dict({
        ...
        'answer': '#5F819D bold',
        'badge': '#5Fff9D italic',
        'end': "fg:#5Fff9D bg: bold underline",
        'default': '',
    })
    ...
    if controller.has_active_choices():
        ...
        print_formatted_text(
            FormattedText(controller._get_formatted_choices()),
            style=style)
        return []
